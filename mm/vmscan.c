...
static unsigned int move_folios_to_lru_opposite(struct lruvec *lruvec,
		struct list_head *list)
{
	int nr_pages, nr_moved = 0;
	struct folio_batch free_folios;
	...
	while (!list_empty(list)) {
		 ...
		 */
		VM_BUG_ON_FOLIO(!folio_matches_lruvec(folio, lruvec), folio);
		lruvec_add_folio_opposite(lruvec, folio);
		nr_pages = folio_nr_pages(folio);
		nr_moved += nr_pages;
		if (folio_test_active(folio))
			workingset_age_nonresident(lruvec, nr_pages);
			       ...
			       enum lru_list lru)
{
	...
	nr_activate = move_folios_to_lru_opposite(lruvec, &l_active);
	nr_deactivate = move_folios_to_lru_opposite(lruvec, &l_inactive);
