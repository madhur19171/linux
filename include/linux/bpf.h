...
struct bpf_map_ops {
	...
	unsigned long (*map_get_unmapped_area)(struct file *filep,
					       unsigned long addr,
					       unsigned long len,
					       unsigned long pgoff,
					       unsigned long flags);
...
int bpf_struct_ops_prepare_trampoline(struct bpf_tramp_links *tlinks,
				      ...
				      u32 *image_off, bool allow_alloc);
...
static inline int bpf_struct_ops_supported(const struct bpf_struct_ops *st_ops,
					   u32 moff)
{
	return -ENOTSUPP;
}
...
#define bpf_map_kzalloc(_map, _size, _flags) kzalloc(_size, _flags)
#define bpf_map_kvcalloc(_map, _n, _size, _flags) kvcalloc(_n, _size, _flags)
...
static inline int sock_map_link_create(const union bpf_attr *attr,
				       struct bpf_prog *prog)
{
	return -EOPNOTSUPP;
}
